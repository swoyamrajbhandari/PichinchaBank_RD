...
|FR~14|The Customer shall be able to transfer money to a contact.|Given that the Customer has at least one saved contact, when the Customer selects a contact from their contact list and selects the "Send New Transfer" button, then the customer is able to complete the RTA form.|Verify that when the customer selects a contact from their contact list and selects the "Send New Transfer" button, then an RTA form is auto-filled with the contact's details and the Customer is prompted to complete the rest of the form.|
